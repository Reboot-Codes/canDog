...
static void candog_scene_write_callback(void* context, CanDogCustomEventWorker event) {
    CanDogApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, event);
}

void candog_scene_write_on_enter(void* context) {
    CanDogApp* app = context;
    candog_view_progress_set_write_callback(
        app->view_progress, candog_scene_write_progress_view_result_callback, app);
    ...
    candog_worker_write_start(app->chip_info, app->worker, candog_scene_write_callback, app);
}

bool candog_scene_write_on_event(void* context, SceneManagerEvent event) {
    CanDogApp* app = context;
    UNUSED(app);
    bool success = false;
...
void candog_scene_write_on_exit(void* context) {
    CanDogApp* app = context;
