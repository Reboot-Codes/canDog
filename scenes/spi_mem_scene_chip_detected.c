...
static void spi_mem_scene_chip_detected_print_chip_info(Widget* widget, SPIMemChip* chip_info) {
    ...
    widget_add_string_element(widget, 40, 12, AlignLeft, AlignTop, FontSecondary, spi_mem_chip_get_vendor_name(chip_info));
    widget_add_string_element(widget, 40, 20, AlignLeft, AlignTop, FontSecondary, spi_mem_chip_get_model_name(chip_info));
...
static void spi_mem_scene_chip_detect_draw_next_button(SPIMemApp* app) {
    ...
    widget_add_button_element(app->widget, GuiButtonTypeRight, furi_string_get_cstr(str), spi_mem_scene_chip_detected_widget_callback, app);
...
void spi_mem_scene_chip_detected_on_enter(void* context) {
    SPIMemApp* app = context;
    widget_add_button_element(app->widget, GuiButtonTypeLeft, "Retry", spi_mem_scene_chip_detected_widget_callback, app);
...
bool spi_mem_scene_chip_detected_on_event(void* context, SceneManagerEvent event) {
    ...
    if (event.type == SceneManagerEventTypeBack) {
        ...
        if (event.event == GuiButtonTypeLeft) {
            scene_manager_search_and_switch_to_previous_scene(app->scene_manager, SPIMemSceneChipDetect);
        }
