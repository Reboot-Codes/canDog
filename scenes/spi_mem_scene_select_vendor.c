...
static void spi_mem_scene_select_vendor_sort_vendors(SPIMemApp* app, vendors_t vendors_arr) {
    for (size_t index = 0; index < found_chips_size(app->found_chips); index++) {
        vendors_push_back(vendors_arr, spi_mem_chip_get_vendor_enum(*found_chips_get(app->found_chips, index)));
    }
...
void spi_mem_scene_select_vendor_on_enter(void* context) {
    ...
    for (size_t index = 0; index < vendors_arr_size; index++) {
        ...
        submenu_add_item(app->submenu, spi_mem_chip_get_vendor_name_by_enum(vendor_enum), vendor_enum, spi_mem_scene_select_vendor_submenu_callback, app);
    }
    vendors_clear(vendors_arr);
    submenu_set_header(app->submenu, "Choose chip vendor");
    submenu_set_selected_item(app->submenu, scene_manager_get_scene_state(app->scene_manager, SPIMemSceneSelectVendor));
